	...
	class Util {
		...
		StartUp() {
			...
			if (Refresh) {
				...
				Paths["/callback"] := this.AuthCallback.Bind(this) ; When someone is redirected by the Spotify auth, run "AuthCallback" and tell AuthCallback to run "FetchTokens" after
		...
		authCallback(self, ByRef req, ByRef res) {
			...
			res.status := 200
			this.AuthCode := req.queries["code"]
			this.AuthFail := req.queries["error"]
		}
		WebAuthDone() {
...
#Include %A_ScriptDir%\includes\lib\AHKsock.ahk
#Include %A_ScriptDir%\includes\lib\AHKhttp.ahk
#Include %A_ScriptDir%\includes\lib\JSON.ahk